...
chmod a+x start.sh 给文件加上可执行权限。ls查看一下，start.sh绿色表示可以执行了 chmod:change mode的简写，用于改变文件或目录的权限。a: 代表 "all"，意味着这个改变适用于所有用户（所有者、组成员和其他人）。+: 表示添加权限。x: 代表可执行（executable）权限。
